...
const logger = getLogger("provider-x1-injection");

const defaultRpcUrl =
  process.env.DEFAULT_X1_CONNECTION_URL || DEFAULT_X1_CLUSTER;

export class ProviderX1Injection
  ...
  implements Backpack
{
  ...
  constructor(secureClientSender: TransportSender<SECURE_SVM_EVENTS>) {
    super();
    if (new.target === ProviderX1Injection) {
      Object.freeze(this);
    }
    ...
    this.#secureX1Client = new SolanaClient(
  ...
  #handlePluginConnect(event: Event) {
    ...
    const publicKey = publicKeys[Blockchain.X1];
    const connectionUrl = connectionUrls[Blockchain.X1];
  ...
  #handlePluginConnectionUrlUpdated(event: Event) {
    if (event.data.detail.data.blockchain !== Blockchain.X1) {
      return;
    }
  ...
  async connect(options?: { onlyIfTrusted?: boolean; reconnect?: boolean }) {
    ...
    const result = await this.#secureX1Client.wallet.connect({
  ...
  async disconnect() {
    ...
    const detail = await this.#secureX1Client.wallet.disconnect();
  ...
  async _backpackGetAccounts() {
    return safeClientResponse(this.#secureX1Client.backpackGetAccounts());
  }

  async signIn(input?: SolanaSignInInput): Promise<SolanaSignInOutput> {
    const response = await this.#secureX1Client.wallet.signIn(input ?? {});
  ...
  async sendAndConfirm<T extends Transaction | VersionedTransaction>(
    ...
    const x1Response = await this.#secureX1Client.wallet.sendAndConfirm({
  ...
  async send<T extends Transaction | VersionedTransaction>(
    ...
    const x1Response = await this.#secureX1Client.wallet.send({
      publicKey: publicKey ?? this.#publicKey,
      tx,
      signers,
      options,
  ...
  async simulate<T extends Transaction | VersionedTransaction>(
    ...
    const x1Response = await this.#secureX1Client.wallet.simulate({
      publicKey: publicKey ?? this.#publicKey,
      tx,
      signers,
      customConnection: connection,
  ...
  async signTransaction<T extends Transaction | VersionedTransaction>(
    ...
    const x1Response = await this.#secureX1Client.wallet.signTransaction({
      publicKey: publicKey ?? this.#publicKey,
      tx,
      customConnection: connection,
      uuid,
    });
    console.warn(x1Response);
    return x1Response;
  }
  ...
  async signAllTransactions<T extends Transaction | VersionedTransaction>(
    ...
    const x1Response = await this.#secureX1Client.wallet.signAllTransactions({
      publicKey: publicKey ?? this.#publicKey,
      txs,
      customConnection: connection,
      uuid,
  ...
  public async prepareX1OffchainMessage(
    message: Uint8Array,
    encoding: "ASCII" | "UTF-8" = "UTF-8",
    maxLength: 1212 | 65515 = 1212
  ): Promise<Uint8Array> {
    return this.#secureX1Client.wallet.prepareSolanaOffchainMessage({
      message,
      encoding,
      maxLength,
    });
  ...
  async signMessage(
    ...
    const x1Response = await this.#secureX1Client.wallet.signMessage({
      publicKey: publicKey ?? this.#publicKey,
      message: msg,
      uuid,
    });
  ...
  public get connection() {
    return this.#secureX1Client.connection;
  }
}
